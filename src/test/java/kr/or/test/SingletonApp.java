 ...
 * 장점. 메모리 낭비를 방지하는 효과.
 * 예, DB접속객체를 예를 들면
 * Connection conn1 = dataSourec.getConnection();//싱글톤 객체 new키워드를 사용X
 * connection 객체가 생성되어 있으면, 
 * Connection conn2 = dataSourec.getConnection();//싱글톤 객체
 * Connection conn3 = dataSourec.getConnection();//싱글톤 객체
 * conn1==conn2==conn3 다 같은 동일객체로 같은 메모리를 사용함. 즉, 1개의 객체만 생성됨.
 * 결과적으로 1번 DB 커넥션이 성공하면, 객체가 없어질 떄 까지 (null) 중복으로 생성되지 않음.
