...
public class CommonUtil {
	...
	public FileSystemResource download(@RequestParam("save_file_name") String save_file_name, @RequestParam("real_file_name") String real_file_name, HttpServletResponse response) throws Exception {
		...
		response.setContentType("application/dowsload; uth-8");//아래 한글, ppt문서등에서 한글내용이 깨지는 것을 방지하는 코드 추가
	...
	public ResponseEntity<byte[]> imagePreview(@RequestParam("save_file_name") String save_file_name, HttpServletResponse response) throws Exception{
		//파일을 입출력할 때는 파일을 byte(이진010001010)형식으로 입출력할때 발생되는 통로  스트림이 발생
		...
		byte[] fileArray = null;//출력스트림 결과 저장하는 공간
		//반복문:목적, fis 입력받는 save_file_name 바이트값이(배열) -1일때 따지 반복
		while((readCount=fis.read(buffer)) != -1) {
			//입력통로fis에서 출력통로 baos보냄.이유는 파일입출력은 byte단위로만 가능.
			baos.write(buffer, 0, readCount);//(rawData, 종료조건, 반복횟수)
			//결과는 baos에 누적된 결과가 들어감. - jsp로 보내주면 됨.
		}
		fileArray = baos.toByteArray();//baos 클래스를 byte[] 배열로 형변환 함.
		fis.close();//메모리 초기화
		baos.close();//메모리 초기화
		//fileArray값을 jsp로 보내주는 준비작업, final 이 메서드에만 사용하겠다고 명시
		final HttpHeaders headers = new HttpHeaders();
		//크롬개발자도구>네트워크>image_preview클릭>헤더탭확인
		String ext = FilenameUtils.getExtension(save_file_name);
		//이미지 확장자에 따라서 매칭되는 헤더값이 변해야지만, 이미지 미리보기가 정상으로 보임.
	...
	//XSS 크로스사이트 스크립트 방지용 코드로 파싱하는 메서드(아래)
	public String unScript(String data) {
		//if(date == null || data.trim().equals("")) {
			if(data.isEmpty()) {
				return "";
		}
			String ret = data;
	...
	public String id_check_2010(@RequestParam("user_id")String user_id, Model model) throws Exception {
		String memberCnt = "1"; //중복ID가 있는 것을 기본값으로 지정.
		if(!user_id.isEmpty()) {
			MemberVO memberVO = memberService.readMember(user_id);
		...
		return "jsonView";//jsp파일명 대신에 servlet에서 정의한 스프링빈 ID명을 적으면, json객체로 결과를 반환 함.
	}
	...
	public String fileUpload(MultipartFile file) throws IOException {
		...
		//file의 MultipartFile클래형 객체. 클래스형 자료는 (멤버변수, 메서드..)직접 저장을 할 수 없음.
		//그래서, 바이트형으로 파싱(변환)해서 저장해야 함. -> bit형(010100001)이진 비트형자료로 변환필요
		//자바자료형 정수: byte(bit로구성)<short<int<long, 실수형(소수점):float<double
		byte[] fileData = file.getBytes();
		File target = new File(uploadPath, saveFileName);
		FileCopyUtils.copy(fileData, target);//물리적으로 폴더에 저장됨
		return saveFileName;//UUID
	}
	public void profile_upload(String user_id, HttpServletRequest request, MultipartFile file) throws IOException {
		...
		String folderPath = request.getServletContext().getRealPath("/resources/profile");//url로 업로드할 수 없기 때문에, 서버 경로를 구하는 명령
		File makeFolder = new File(folderPath);//공백인 객체가 생성
		if(!makeFolder.exists()) {
			makeFolder.mkdir();//여기서 신규 폴더가 생성됨
		}
		//1개에 파일이 1000개 이상이면, 조회속도가 엄청 느려짐.
		//년월일 폴더를 생성 후, 해당년월에 업로드된 파일은 년월 폴더로 관리.
		byte[] fileData = file.getBytes();
		File target = new File(makeFolder,user_id+".png");//user_id는 PK이기 때문에 기존파일이 있다면, 덮어쓰면서 저장.
		FileCopyUtils.copy(fileData, target);//첨부파일이 저장이 됨.
	}
