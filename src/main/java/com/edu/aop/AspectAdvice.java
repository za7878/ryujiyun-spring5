...
public class AspectAdvice {
	...
	//아래 조인포인트는(진입점) board_delete || board_update* 메서드를 실행할 때,
	//본인이 작성한 글인지 확인하는 기능.(단, ROLE_Admin 사용자는 제외)을 만듬.
	...
	public Object check_board_crud(ProceedingJoinPoint pjp) throws Throwable {
		...
		// 사용 할 변수 초기화(null)
		String user_id = null;//게시물 작성자 ID변수
		BoardVO boardVO = null;//boardVO.getWriter()게시물의 작성자 ID를 구하려면 필요한 객체 변수
		logger.info("디버그 호출된 메서드 명은: "+pjp.getSignature().getName());
		//향상된 for문을 이용해서 매개변수 중 bno, boardVO 2가지를 체크해서 user_id를 구함.
		for(Object object:pjp.getArgs()) {
			//메서드의 매개변수가 BoardVO, bno일 때만 로직을 실행하기 위해서
			...
			if(object instanceof Integer) {//bno일때는 Integer형 데이터 형임.
		...
		if(request != null) {
			...
			//위 메서드의 매개변수를 이용해서 생성된 user_id와 세션 user_id와 비교xROLE_USER일때 조건추가.
			if(!user_id.equals(session.getAttribute("session_userid")) && "ROLE_USER".equals(session.getAttribute("session_levels"))) {
				...
				//위 RedirectAttribute를 사용하는 대신 Flash(휘발성 객체 1번 사용 후, 사라짐.) 클래스
		...
		Object result = pjp.proceed();//실제 조인포인트 실행이 여기서 됨.
		return result;
	}
	...
	//이 메서드는 컨트롤러에서 Exception이 발생했을 때 여기서 인터셉터(가로채기)해서
	//에러 메세지를 개발자가 작성한 jsp화면에 뿌려주는 기능을 추가 prevPage 변수 1, exception 변수2 전송함.
	...
	request.getSession().setAttribute("session_prevPage", referer);//prevPage세션변수 만듬.
	//-----------------------------------------------
