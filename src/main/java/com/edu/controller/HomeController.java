...
public class HomeController {
	...
	public String board_update(HttpServletRequest request, @RequestParam("file")MultipartFile[] files,BoardVO boardVO,PageVO pageVO, RedirectAttributes rdat) throws Exception {
		...
		if(!boardVO.getWriter().equals(session.getAttribute("session_userid"))) {
			...
			//request객체에 이전 페이지 URL("Referer")로 존재하고, 이 URL이 이전페이지 이동으로 사용
			return "redirect:"+request.getHeader("Referer"); //본인 ID의 글이 아닐때 뷰페이지로 이동
		}//else로 묶을 필요가 없음. 왜? 위 return을 만나면, 이후 실행이 안되고 메서드가 종료됨.
		*/
		//첨부파일 처리
	...
	*/
	...
	public String board_view(Model model,@RequestParam("bno")Integer bno,@ModelAttribute("pageVO")PageVO pageVO) throws Exception {
		...
		int index = 0;
		for(AttachVO file:listAttachVO) {//새로데이터를 가롸데이터로 변경처리
				save_file_names[index] = file.getSave_file_name();
				real_file_names[index] = file.getReal_file_name();
				index = index + 1;	
		}
	...
	public String board_insert(RedirectAttributes rdat, @RequestParam("file")MultipartFile[] files,BoardVO boardVO) throws Exception{
		...
		rdat.addFlashAttribute("msg", "게시물 등록");//출력:게시물 등록이(가) 성공했습니다.
	...
	public String join(MemberVO memberVO, RedirectAttributes rdat) throws Exception {
		...
		//사용자 레벨은 UI단에서 보내는 값 무시하고 강제로 입력(해킹 위험 때문)
	...
	//네아로 로그인 때문에 LoginController클래스로 분리해서 사용. 그래서 아래는 주석처리
...
//		logger.info("디버그 스프링로고사용: " + jspVar);//System.out 대신 logger 객체를 사용
		...
		pageVO.setQueryPerPageNum(3);//갤러리3개
		...
		pageVO.setQueryPerPageNum(5);//공지사항5개, 보드타입 필요(세션으로 처리 안됨.)
		...
		return "index.tiles";//타일즈 적용 후. tiles폴더 안에 index.jsp 실행
	}
