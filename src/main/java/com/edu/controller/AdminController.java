...
public class AdminController {
	...
	public String board_inset_form(@ModelAttribute("pageVO")PageVO pageVO) throws Exception {
	...
	public String board_update(@RequestParam("file")MultipartFile[] files,BoardVO boardVO, PageVO pageVO) throws Exception {
		//기존 등록된 첨부파일 목록 구하기 List(2차원배열)객체와 크기는 .size() 구함. 기존파일이 있을 때 사용.
		...
		for(MultipartFile file:files) {//files[0]=file, files[1]
			if(file.getOriginalFilename() != "") {//전송된 첨부파일명이 있다면 실행
				...
				//아래 for목적: jsp폼에서 기존에 1번 위치에 기존 파일이 있으면, 기존파일을 지우고, 신규파일 덮어쓰는 로직.
				for(AttachVO file_name:delFiles) {//기존파일을 가져와서 반복하면서 지우기 로직
		...
		//첨부파일 작업전, 시큐어코딩 : 입력/수정시 시큐어코딩 적용O, 뷰 화면에서 시큐어X
		
		String queryString = "bno="+boardVO.getBno()+"&page="+pageVO.getPage()+"&search_type="+pageVO.getSearch_type();
		return "redirect:/admin/board/board_view?"+queryString;//수정한 이후에는 board_view페이지로
		//이동:새로고침 방지하기 위해서 redirect사용
	}
