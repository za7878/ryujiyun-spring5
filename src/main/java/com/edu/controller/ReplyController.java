...
public class ReplyController {
	...
	//댓글은 Read가 필요없음. 왜냐하면, Select로 가져온 값을 Ajax로 처리하기 때문에 쿼리를 날릴 필요가 없음.
	//그래서, 바로 Update를 처리함. - 간단하게 update시 Read쿼리가 없고, Ajax처리함.
	...
	public ResponseEntity<String> reply_update(@RequestBody ReplyVO replyVO) {
		//@RequestBody jsp에서 $.ajax로 보내는 데이터값 <-> @ResponseBody 응답 return값
		ResponseEntity<String> result = null;
		try {
	...
	public ResponseEntity<String> reply_insert(@RequestBody ReplyVO replyVO) {
		...
		ResponseEntity<String> result = null;
		//개발자가 스프링에 예외를 throws 하지 않고, 직접 처리 try~catch하는 목적:
		//Rest상태값(200,204,500등의 상황들)을 개발자가 보내줘야 하기 때문에.
		try {
			replyService.insertReply(replyVO);
			result = new ResponseEntity<String>("success",HttpStatus.OK); //객체 생성시, 매개변수로 상태값 + 입력 성공시 success라는 문자열도 보냄.
