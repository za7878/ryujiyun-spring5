        ...
        <ul class="bbsview_list">
            ...
            <c:forEach begin="0" end="1" var="idx">
            	...
            	<c:url var="url" value="/download">
            		...
            		<!-- 위 처럼 c:url로 쿼리스트링을 처리하면 한글이 인코딩 되어서 전송됨(한글 안깨짐) -->
            		<a href="${url}">다운로드
            		${boardVO.real_file_names[idx]}
            		</a>
            		<!-- 만약 첨부파일이 jpg, jpeg, gif, png, bmp라면 img태그를 사용해서 미리보기 기능 추가 -->
...
<!-- 모달창(초기엔 숨긴상태model-display:none, fade-opacity:0-수정버튼을 클릭하면 나타나는 창) -->
...
//pageVO = 스프링에서 받은 json데이터, 변수3개 pageVO.prev(이전데이터가 있다면 true), pageVO.next(다음데이터 있다면 true), pageVO=5페이지로 가정
	...
	pagination += '<li class="paginate_button page-item previous '+nextlink+' disabled" id="example2_previous">';
...
var replyList = function() {
	...
	$.ajax({
		...
		error:function(result) {
			...
			//전체 json데이터를 출력할 때 stringify함수로 형변환해서 출력
			//만약 json데이ㅓㅌ에서 키이름을 알게 되면, stringify함수 필요 없이 result.responseText 출력가능
			alert(result.responsText);//이클립스에서 확인할 때
...
$(document).ready(function(){
	...
	$("#btn_reply_delete").click(function(){
		//댓글을 삭제할 때 필요한 변수확인 2개 rno(삭제쿼리 사용), bno(게시물 댓글카운트 업데이트)
		...
		$.ajax({
			...
			//data:"",//처리할 값을 보내는 데이터형식 -> json을 사용하지 않고, 패스베리어블 보내기 때문.
			//headers:"",//크롬의 개발자도구>네트워크항목의 오른쪽 창에서 확인가능, 전송 방식 때문에 필요.
			success:function(result) {
				if(result == "success") {
					alert("삭제 되었습니다.");
					//삭제후 모달창 숨기고, 댓글카운트UI -1처리, 댓글 리스트 리프레쉬(렌더링)
	...
	$("#btn_reply_write").click(function(){
		...
		$.ajax({
			...
			complete:function(){
				//success 완료된 이후 실행해야 할 내용이 있을 때 사용
				//success에서 함수가 여러개 실행될 때 순서가 중요한 경우 분리해서 사용.
				$("#reply_text").val("");//기존 입력창의  댓글 내용을 지우기.
				alert("등록이 완료 되었습니다.");
			},
			asycn:true
			//Asycn++Javascript And Xml:비동기 데이터 통신
			//Ajax이지만, 비동기 False로 하는 경우(아이작스로 대용량 첨부파일 업로드 할 때)
			//success:업로딩 중.... 표시가 됨. -> 업로드가 완료 되면 complete실행됨.
		});
	});
});
</script>
